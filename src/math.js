...
function mat4IsometricView(out) {
    const yaw = Math.PI / 4, pitch = Math.atan(1 / Math.sqrt(2));
...
function calculateIsometricFitBounds(roomSize, roomHeight, marginPercent = 0.15) {
    ...
    const corners = [
        ...
        [-halfSize, roomHeight, -halfSize], [halfSize, roomHeight, -halfSize], [halfSize, roomHeight, halfSize], [-halfSize, roomHeight, halfSize]
    ];
    const yaw = Math.PI / 4, pitch = Math.atan(1 / Math.sqrt(2));
    const cy = Math.cos(yaw), sy = Math.sin(yaw), cp = Math.cos(pitch), sp = Math.sin(pitch);
    let minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity;
    for (const [x, y, z] of corners) {
        const tx = cy * x + sy * z, ty = sy * sp * x + cp * y - cy * sp * z;
        minX = Math.min(minX, tx); maxX = Math.max(maxX, tx);
        minY = Math.min(minY, ty); maxY = Math.max(maxY, ty);
    }
    const margin = 1 + marginPercent;
    return { spanX: (maxX - minX) * margin, spanY: (maxY - minY) * margin, centerX: (minX + maxX) / 2, centerY: (minY + maxY) / 2 };
}

window._viewMatrix = _viewMatrix;
window._projMatrix = _projMatrix;
window.mat4Orthographic = mat4Orthographic;
window.mat4IsometricView = mat4IsometricView;
window.mat4Identity = mat4Identity;
window.calculateIsometricFitBounds = calculateIsometricFitBounds;
